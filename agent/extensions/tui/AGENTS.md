...
- `tools/register.ts` — `registerToolRenderers(pi)`: re-registers each built-in tool (`read/write/edit/bash/ls/grep/find/powershell`) with the unified per-tool renderers while preserving execution by delegating `execute` to the host `create*Tool` factories (powershell reuses a spawn mirror of `extensions/powershell`), wrapping results to expose `details._fullOutput`. Called from `index.ts` after `registerBar`.
