...
- `tools/register.ts` — `registerToolRenderers(pi)`: re-registers `read/write/edit/ls/grep/find` with the unified per-tool renderers (delegating `execute` to the host `create*Tool` factories) and calls `registerExecuteTool(pi)` so `bash`/`pwsh`/`powershell` route to the combined Execute tool. Results are wrapped to expose `details._fullOutput`. At the end it calls `patchUnknownToolRenderers(pi)` (see `tools/patch-tools.ts`) so any tool without a dedicated render module still renders unified. Called from `index.ts` after `registerBar`.
- `tools/patch-tools.ts` — `patchUnknownToolRenderers(pi)` (mirrors the archived `compactui` monkey-patch pattern, guarded by a `try/catch` + idempotent `__tui_patchTools_patched` global flag): (1) patches `ToolExecutionComponent.prototype.render` so unknown/generic tools (not in `DEDICATED_TOOLS`) render the unified block via `unifiedBlock` (greyish-white tool title + `↳` muted summary + full-width `─` separator) instead of a raw unstyled dump, returning `[]` for `SUPPRESSED_TOOLS`; (2) patches BOTH the instance `registerTool` and the prototype `registerTool` so late-registered tools (subagent, memory, video_extract, future tools from other extensions) are also given `renderShell: "self"` + unified `renderCall`/`renderResult` (using the host theme) at registration time; (3) patches `InteractiveMode.addMessageToChat` to install a one-time `chatContainer.addChild` wrapper that injects exactly one `Spacer(1)` above each non-blank child for uniform spacing (no double spacers). Known/dedicated tools and tools that ship their own `renderShell: "self"` renderer are left untouched. `DEDICATED_TOOLS` = `read/write/edit/ls/grep/find/bash/pwsh/powershell`; `SUPPRESSED_TOOLS` = `todo`.
