...
export async function createPreviewServerState(
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      const cookieToken = cookies["pi_artifact_token"];

      const isAuthenticated = tokenParam === authToken || cookieToken === authToken;

      if (!isAuthenticated) {
        sendText(response, 403, "Forbidden: Invalid or missing authentication token");
        return;
      }
      ...
      if (tokenParam === authToken) {
        response.setHeader(
          "Set-Cookie",
          `pi_artifact_token=${authToken}; HttpOnly; SameSite=Lax; Path=/`
        );
      }
  ...
  return {
    url: `${baseUrl}?token=${authToken}`,
    viewerUrl: `${baseUrl}/viewer?token=${authToken}`,
    ...
    artifactUrl(id) {
      return `${baseUrl}/artifacts/${encodeURIComponent(id)}/?token=${authToken}`;
    },
    setSessionKey(key) {
      sessionKey = key;
    },
...
function broadcast(
  ...
  if (artifactId) payload.id = artifactId;
...
async function sendViewer(
  ...
  const filtered = scoped.filter((artifact) => {
...
<header>
<p>${filtered.length} of ${scoped.length} artifact${scoped.length === 1 ? "" : "s"}
