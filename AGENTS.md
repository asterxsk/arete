...
- **Docs**: `docs/architecture.md` (extension loading, global bridges, external deps via mermaid) and `docs/secrets.md` (env-based secrets, CI `secrets.*` usage, Gitleaks + runtime scanning).
- **Branch protection**: `main` requires passing CI checks (lint, format:check, typecheck, test, secret-scan, duplicate-check) and one approving review; enforced for admins. Release notes auto-generated by Release Please.
